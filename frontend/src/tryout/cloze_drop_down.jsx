...
function QuestionCard({ question, options }) {
    ...
    return (
        <div className="w-3/4 border flex flex-col items-center justify-center gap-5 border-purple-500 p-8 rounded-xl shadow-lg">
            <h2 className="text-xl font-bold text-black self-start">Fill in the blanks from the drop down:</h2>
            <div>
                {parts.map((part, i) => (
                    <React.Fragment key={i}>
                        <span className="select-none text-black">{part}</span>
                        {i < parts.length - 1 && (
                            <select className="border-2 p-2 border-purple-500 focus:outline-none w-24 text-black">
