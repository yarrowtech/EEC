      ...
      <div className={`w-3/4 grid grid-cols-3 grid-rows-${items.length} gap-y-3`}>
        {items.map((item, i) => {
          return (
            ...
            <p
              className="min-h-10 p-2 min-w-10 rounded-lg border box-border border-purple-500 text-black"
              >
              {item}
            </p>
            <hr key={`line-${i}`} className="self-center" />
            <p
              className={`min-h-10 p-2 min-w-10 rounded-lg border border-gray-300 box-border ${showMatch[i].show ? "bg-yellow-100 flex justify-between border-purple-500" : ""} text-black`}
